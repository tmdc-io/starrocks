...
public class AuditStreamLoader {
    ...
    public AuditStreamLoader() {
        this.hostPort = "127.0.0.1:8030";
        this.user = AuthenticationMgr.ROOT_USER;
        this.passwd = "";
    ...
    private HttpURLConnection getConnection(String urlStr, String label) throws IOException {
        ...
        conn.addRequestProperty("columns", "queryId,timestamp,queryType,clientIp,user,authorizedUser,resourceGroup,catalog,db," +
                "state,errorCode,queryTime,scanBytes,scanRows,returnRows,cpuCostNs,memCostBytes,stmtId,isQuery,feIp,stmt," +
                "digest,planCpuCosts,planMemCosts,pendingTimeMs,candidateMVs,hitMvs,warehouse");
    ...
    private String toCurl(HttpURLConnection conn) {
        ...
        sb.append("-H \"")
                .append("columns\":")
                .append("\"queryId, timestamp, queryType, clientIp, user, authorizedUser, resourceGroup, catalog, db, state, " +
                        "errorCode,queryTime, scanBytes, scanRows, returnRows, cpuCostNs, memCostBytes, stmtId, isQuery, " +
                        "feIp, stmt, digest, planCpuCosts, planMemCosts, " +
                        "pendingTimeMs, candidateMVs, hitMvs, warehouse\" \\\n  ");
